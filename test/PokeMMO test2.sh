...
if [ "$westonpack" -eq 1 ]; then 
$ESUDO env CRUSTY_SHOW_CURSOR=1 SDL_NO_SIGNAL_HANDLERS=1 WRAPPED_LIBRARY_PATH="$GAMEDIR/libs:$LD_LIBRARY_PATH" WESTON_HEADLESS_WIDTH="$DISPLAY_WIDTH" WESTON_HEADLESS_HEIGHT="$DISPLAY_HEIGHT" $weston_dir/westonwrap.sh headless noop kiosk crusty_glx_gl4es \
PATH="$PATH" JAVA_HOME="$JAVA_HOME" XDG_DATA_HOME="$GAMEDIR" \
java -Xms128M -Xmx384M -Dorg.lwjgl.util.Debug=true -Dfile.encoding="UTF-8" -cp "PokeMMO.exe" com.pokeemu.client.Client
#Clean up after ourselves
$ESUDO $weston_dir/westonwrap.sh cleanup
else
PATH="$PATH" CRUSTY_SHOW_CURSOR=1 JAVA_HOME="$JAVA_HOME" XDG_SESSION_TYPE="x11" java -Xms128M -Xmx384M -Dorg.lwjgl.util.Debug=true -Dfile.encoding="UTF-8" -cp "PokeMMO.exe" com.pokeemu.client.Client
