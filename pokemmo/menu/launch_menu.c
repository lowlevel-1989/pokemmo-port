...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(argv[i], "--godot") == 0) {
      ...
      if (i + 1 < argc) {
        ...
        fprintf(stderr, "ERR: --show requires an argument.\n");
        return 1;
      }
    }else if (!menu_file) {
      menu_file = argv[i];
  ...
  if (show_message) {
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    SDL_RenderClear(renderer);
    render_text(renderer, font, show_message, 16, 16, WHITE);
    SDL_RenderPresent(renderer);
    SDL_Delay(3000);
    return 0;
  }
  ...
  SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
  SDL_RenderClear(renderer);
  render_text(renderer, font, "Loading", 16, 16, WHITE);
  SDL_RenderPresent(renderer);
  SDL_Delay(300);
